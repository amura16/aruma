...
const Comment = ({ id, user_id, author, content, created_at, replies = [] }) => {
  ...
  const handleReplySubmit = async (e) => {
    ...
    setIsSending(true);
    try {
      await addReply(id, `@${author?.username} ` + replyText.trim());
      setReplyText("");
            ...
            {isMyComment && !isEditing && (
              <div className="flex items-center gap-1 opacity-0 group-hover/bubble:opacity-100 transition-opacity">
                <button 
                  ...
                  className="p-1.5 hover:bg-gray-200 rounded-full text-gray-500 hover:text-blue-600"
                >
                  <Edit2 size={14} />
                </button>
                <button 
                  onClick={() => { if (window.confirm("Supprimer ?")) deleteComment(id); }} 
                  className="p-1.5 hover:bg-gray-200 rounded-full text-gray-500 hover:text-red-500"
                >
                  <Trash2 size={14} />
                </button>
              </div>
          ...
          <form onSubmit={handleReplySubmit} className="flex items-center gap-2">
            <div className="relative flex-1">
              <input
                autoFocus
                disabled={isSending}
                className="w-full bg-white border border-gray-200 rounded-full py-1.5 px-4 text-xs outline-none focus:ring-1 focus:ring-blue-400"
