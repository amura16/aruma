...
import { Heart, MessageCircle, Share2, MoreHorizontal, Send, Edit2, Trash2, X, Check } from 'lucide-react';
...
const PostCard = ({
  ...
  parent_post // Contient les données du post d'origine si c'est un partage
}) => {
  ...
  // 1. Fermeture automatique du menu d'options (clic ailleurs, scroll ou touch)
  useEffect(() => {
    ...
    <div className="bg-white border border-gray-200 rounded-xl mb-4 overflow-hidden shadow-sm relative">
      ...
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-[14px] text-gray-900 leading-tight">{authorName}</h4>
        ...
        {isMe && (
          <div className="relative">
            ...
            {showOptions && (
              <div
                className="absolute right-0 mt-2 bg-white shadow-xl border border-gray-100 rounded-xl py-2 w-48 z-[100] animate-in fade-in zoom-in-95 duration-100 origin-top-right"
        ...
        {isEditing ? (
          <div className="space-y-2">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsEditing(false)} className="p-1.5 text-gray-500"><X size={18} /></button>
              <button onClick={handleUpdatePost} className="p-1.5 text-blue-600"><Check size={18} /></button>
            </div>
          </div>
        ) : (
          <p className="text-[15px] text-gray-800 whitespace-pre-wrap leading-snug">{content}</p>
        )}
      </div>

      {/* 2. AFFICHAGE DU POST PARTAGÉ (Si parent_post existe) */}
      {parent_post && (
        <div
          onClick={(e) => { e.stopPropagation(); navigate(`/post/${parent_post.id}`); }}
        ...
        >
          <div className="p-3 flex items-center gap-2 border-b border-gray-50 bg-gray-50/30">
            <img src={parent_post.author?.avatar_url} className="w-5 h-5 rounded-full" alt="origin" />
      ...
      {/* IMAGE DU POST (Seulement si ce n'est pas un partage) */}
      {image_url && !parent_post && !isEditing && (
      ...
      {/* STATS & ACTIONS (Spécifiques à ce post-id) */}
      ...
      <div className="px-2 py-1 flex items-center justify-between border-b border-gray-50">
        <button onClick={() => toggleLike(id, isLikedByMe)} className={`flex-1 flex items-center justify-center gap-2 py-2 font-bold text-sm rounded-lg transition ${isLikedByMe ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      {showComments && (
        <div className="bg-gray-50/50 p-4">
          <form onSubmit={(e) => { e.preventDefault(); if (commentText.trim()) { addComment(id, commentText); setCommentText(""); } }} className="flex gap-2 mb-6">
            <input
              type="text"
              placeholder="Écrire un commentaire..."
              value={commentText}
              onChange={(e) => setCommentText(e.target.value)}
              className="w-full bg-white border border-gray-200 rounded-full py-1.5 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all"
            />
      ...
      {/* MODAL DE PARTAGE (On passe le post entier comme base de partage) */}
      {showShareModal && (
        <SharePostCard
